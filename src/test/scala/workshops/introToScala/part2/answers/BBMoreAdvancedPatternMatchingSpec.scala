...
class BBMoreAdvancedPatternMatchingSpec extends UnitSpec{
  "a scala adept" should {
    "be able to create simple apply" in {
      ...
      object MyFileReader{
        def apply(path: String): MyFileReader = new MyFileReader(new MyFile(path))
      }
    ...
    "be able to create simple unapply" in {
      ...
      object SimpleClass{
        def unapply(arg: SimpleClass): Option[String] = Some(arg.arg)
      }
    ...
    "be able to nicely hide implementation details using apply and unapply" in {
      ...
      object MyAtomicClass{
        def apply(initialValue: Int): MyAtomicClass = new MyAtomicClass(new AtomicInteger(initialValue)) // new MyAtomicClass(initialValue)
        def unapply(arg: MyAtomicClass): Option[Int] = Some(arg.handler.get())
      }
    ...
    "be able to prepare unapply for more complex class" in {
      ...
      object User{
        def unapply(arg: User): Option[String] =
          if(arg.fullName != " ") Some(arg.fullName)
          else None
      }
