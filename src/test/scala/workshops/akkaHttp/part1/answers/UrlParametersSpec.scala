...
class PadlocksController(initialList: Array[Padlock] = Array()){
  ...
  val route : Route = pathPrefix("padlock"){
    path("new" / Segment / Segment) { (p1name,p2name) =>
      post {
        padlocks = padlocks ++ List(Padlock(p1name, p2name))
        complete(StatusCodes.OK)
      }
    } ~
    path(IntNumber) { index =>
      get {
        Try(padlocks(index -1)) match {
          case scala.util.Success(p) => complete(serializePadlock(p))
          case scala.util.Failure(_) => complete(StatusCodes.NotFound)
        }
      }
    }
  }
