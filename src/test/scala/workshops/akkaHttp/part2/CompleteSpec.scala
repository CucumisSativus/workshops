...
class CompleteSpec extends UnitSpec with ScalatestRouteTest{
  ...
  "Post service" should {
    ...
    "return 10 record from the second page if both per page and page parameters are given" in {
      ...
      Get("/posts?perPage=25&page=2") ~> route ~> check{
...
object CompleteSpec{
  ...
  class PostsController(service: PostService){
    ...
    def route: Route =
       showRoute ~ createRoute //~ indexRoute

    private def showRoute: Route = path("post" / IntNumber){ index =>
    ...
    private def createRoute: Route = path("posts"){
