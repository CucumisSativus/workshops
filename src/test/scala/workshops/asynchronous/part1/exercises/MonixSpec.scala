...
import workshops.asynchronous.part1.exercises.MonixSpec.MyRequestService
...
private[exercises] object MonixSpec{
  class MyRequestService{
    val testService = new TestService

    def delayedTask: Task[Int] =  ??? //testService.methodWhichReturnProperValueAfterSomeTime
    def retryTask: Task[Int] =  ??? //testService.methodWhichReturnsProperValueAfterSecondCall
    def composeTask(value: Int): Task[String] = ??? // compose taskToBeComposed1 -> taskToBeComposed2
